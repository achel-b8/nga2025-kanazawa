...
describe('メインページ（2024年版）', () => {
	...
	it('LINEアカウント情報が表示される', () => {
		...
		const lineLinks = Array.from(container.querySelectorAll('a')).filter(
			(link) =>
				link.getAttribute('href')?.includes('line.me') ||
				link.getAttribute('href')?.includes('lin.ee')
		);
