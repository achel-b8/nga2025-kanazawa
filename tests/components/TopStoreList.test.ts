...
import TopStoreList from '../../src/components/TopStoreList.svelte';
...
describe('TopStoreList', () => {
	it('複数の店舗が正しく表示される', () => {
		...
		const storeRows = container.querySelectorAll(
			'div.bx--structured-list-row:not(.bx--structured-list-row--header-row)'
		);
